...
Note: The _generated/api.ts file is auto-generated by Convex and should not be manually modified.
  </action>
  <verify>
```bash
...
# Regenerate Convex types (if dev server not running)
cd apps/convex && npx convex dev --once 2>/dev/null || echo "Run 'npx convex dev' to regenerate types"
```
  </verify>
