...
2. **getIndexedJurisdictions(index: Index, sourceType: string): Promise<string[]>**
   ...
   - Use a dummy query vector (3072-dim zeros) with topK=10000 and filter by sourceType
   - **Deduplication approach:**
     - Query returns matches array: `{ id, score, metadata }[]`
     - Extract jurisdiction from each match: `matches.map(m => m.metadata?.jurisdiction)`
     - Dedupe using Set: `[...new Set(jurisdictions.filter(Boolean))]`
     - Return unique jurisdiction strings
   - Note: Pinecone doesn't have distinct(). topK=10000 is a sample; production may have more vectors.
     For comprehensive coverage, consider paginating with multiple queries or tracking indexed jurisdictions separately.
