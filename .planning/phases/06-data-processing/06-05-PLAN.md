...
must_haves:
  truths:
    ...
    - "R2 storage structure validated for all indexed jurisdictions"
    - "Convex jurisdictions table sync validated (lastScrapedAt, status, vectorCount)"
  artifacts:
    - path: "apps/workers/src/validation/quality-reporter.ts"
      provides: "Data quality report generation"
      exports: ["generateQualityReport", "formatQualityReportMarkdown", "generateFullValidationReport", "validateR2Storage", "validateConvexSync"]
  key_links:
    ...
    - from: "apps/workers/src/validation/quality-reporter.ts"
      to: "R2Bucket"
      via: "R2 storage validation"
      pattern: "r2\\.list|R2Bucket"
    - from: "apps/workers/src/validation/quality-reporter.ts"
      to: "ConvexHttpClient"
      via: "Convex sync validation"
      pattern: "ConvexHttpClient|convexClient\\.query"
---
...
7. **validateR2Storage(r2: R2Bucket, indexedJurisdictions: string[]): Promise<R2ValidationResult>**
   - Validates R2 folder structure for indexed jurisdictions
   - Check existence of folders for each sourceType: `federal/`, `state/`, `county/`, `municipal/`
   - For each indexed jurisdiction, verify R2 has data (list objects with prefix)
   - Return: { valid: boolean, missingFolders: string[], jurisdictionsWithoutData: string[] }
   - R2ValidationResult type: add to imports from './types'

8. **validateConvexSync(convexClient: ConvexHttpClient, indexedJurisdictions: string[], pineconeCounts: Map<string, number>): Promise<ConvexValidationResult>**
   - Query Convex jurisdictions table for all indexed jurisdiction IDs
   - Verify each has:
     - lastScrapedAt is set (not null/undefined)
     - status is 'active' (for successfully processed jurisdictions)
     - vectorCount approximately matches Pinecone counts (within 10% tolerance)
   - Return: { valid: boolean, missingInConvex: string[], staleJurisdictions: string[], vectorCountMismatches: Array<{jurisdiction: string, convex: number, pinecone: number}> }
   - ConvexValidationResult type: add to imports from './types'
...
- validateR2Storage verifies folder structure for federal/state/county/municipal
- validateConvexSync confirms jurisdictions have lastScrapedAt, status='active', matching vectorCount
